...
impl AIService {
    ...
    async fn call_baidu(&self, prompt: &str) -> Result<String, String> {
        let url = match &self.config.base_url {
            Some(url) if !url.is_empty() => url.clone(),
            _ => "https://aip.baidubce.com/rpc/2.0/ai_custom/v1/wenxinworkshop/chat/completions".to_string(),
        };
...
",
        ...
        let resistance_level = if report.technical.ma20 > 0.0 {
            format!("约{:.1}-{:.1}元区域", report.technical.ma20, report.technical.ma10.max(report.technical.ma20))
        } else {
            "需要结合更多技术指标判断".to_string()
        };
        ...
        let current_price_status = if report.technical.bb_position < 0.3 { "低位" } else { "中高位" };
        let rsi_status = if report.technical.rsi < 30.0 { "超卖" } else if report.technical.rsi > 70.0 { "超买" } else { "正常" };
        let rebound_chance = if report.technical.rsi < 30.0 { "增加" } else { "适中" };
        let valuation_status = if let (Some(pe), Some(pb)) = (pe_ratio, pb_ratio) {
            if pe < 10.0 && pb < 1.0 { "低" } else if pe < 20.0 && pb < 2.0 { "合理" } else { "偏高" }
        } else {
            "需要更多数据评估"
        };
        let safety_margin = if let (Some(pe), Some(pb)) = (pe_ratio, pb_ratio) {
            if pe < 10.0 && pb < 1.0 { "高" } else if pe < 20.0 && pb < 2.0 { "中等" } else { "低" }
        } else {
            "需要更多数据评估"
        };
        let dividend_status = dividend_yield.map_or("需要数据评估".to_string(), |dy| {
            if dy > 3.0 { "高".to_string() } else if dy > 1.5 { "适中".to_string() } else { "低".to_string() }
        });
        let trend_pressure = if ma_trend_desc == "空头排列" { "空头排列" } else { "当前趋势" };
        let pressure_type = if ma_trend_desc == "空头排列" { "压制" } else { "支撑" };
        let macd_status = if report.technical.macd_signal == "横盘整理" { "横盘整理" } else { "信号" };
        let volume_coordination = if report.technical.volume_status == "放量" { "充足" } else if report.technical.volume_status == "缩量" { "不足" } else { "一般" };
        ...
        let company_news_count = report.sentiment.news_distribution.get("company_news").unwrap_or(&0);
        ...
        if report.scores.comprehensive >= 50.0 {
            let rsi_text = if report.technical.rsi < 30.0 { "超卖" } else { "当前" };
            let bb_text = if report.technical.bb_position < 0.3 { "接近下轨" } else { "相对稳定" };
            let _valuation_text = if let (Some(pe), Some(pb)) = (pe_ratio, pb_ratio) {
                if pe < 15.0 && pb < 1.0 { "低位" } else if pe < 25.0 && pb < 2.0 { "合理水平" } else { "高位" }
            } else {
                "需要评估"
            };
            ...
            let dividend_text = dividend_yield.map_or("需要数据评估".to_string(), |dy| {
                if dy > 3.0 { "高达".to_string() } else if dy > 1.5 { "适中".to_string() } else { "偏低".to_string() }
            });
            let sentiment_text = if report.sentiment.overall_sentiment > 0.1 { "偏向积极" } else { "相对稳定" };
...
主要风险包括：{}。建议止损价位设置在当前价位的{}以下。目标价位区间为{}，风险收益比约为{}。".to_string()
    ...
    async fn stream_provider_analysis(
        ...
        let result = match provider {
            ...
            _ => {
                Ok(format!("{} 流式分析暂未实现，使用模拟数据。", provider))
            }
        };
