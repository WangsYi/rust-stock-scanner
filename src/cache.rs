...
impl DataCache {
    ...
    fn start_cleanup_task(&self) {
        ...
        let _cleanup_task = tokio::spawn(async move {
            let mut interval_timer = tokio::time::interval(tokio::time::Duration::from_secs(interval as u64));
    ...
    pub async fn set_price_data(&self, stock_code: &str, days: i32, data: Vec<PriceData>) {
        ...
        if self.config.enable_stats {
            let mut stats = self.stats.write().await;
            stats.total_entries = cache.len() + self.fundamental_cache.read().await.len()
                + self.news_cache.read().await.len() + self.name_cache.read().await.len();
        }
    }
            ...
            if !entry.is_expired() {
                ...
                if self.config.enable_stats {
                    ...
                    stats.total_entries = self.price_cache.read().await.len() 
                        + cache.len() + self.news_cache.read().await.len()
    ...
    pub async fn set_fundamental_data(&self, stock_code: &str, data: FundamentalData) {
        ...
        if self.config.enable_stats {
            let mut stats = self.stats.write().await;
            stats.total_entries = self.price_cache.read().await.len() + cache.len()
                + self.news_cache.read().await.len() + self.name_cache.read().await.len();
        }
    }

    pub async fn get_news_data(&self, stock_code: &str, days: i32) -> Option<(Vec<News>, SentimentAnalysis)> {
            ...
            if !entry.is_expired() {
                ...
                if self.config.enable_stats {
                    ...
                    stats.total_entries = self.price_cache.read().await.len() 
                        ...
                        + cache.len() + self.name_cache.read().await.len();
                }
            }
        }
    ...
    pub async fn set_news_data(&self, stock_code: &str, days: i32, data: (Vec<News>, SentimentAnalysis)) {
        let key = self.generate_news_key(stock_code, days);
        let mut cache = self.news_cache.write().await;
        ...
        if let Some(entry) = cache.get_mut(&key) {
            if !entry.is_expired() {
                ...
                if self.config.enable_stats {
                    ...
                    stats.total_entries = self.price_cache.read().await.len() 
                        + self.fundamental_cache.read().await.len()
                        + self.news_cache.read().await.len() + cache.len();
                }
            }
        }
    ...
    async fn evict_lru_fundamental_cache(&self, cache: &mut HashMap<String, CacheEntry<FundamentalData>>) {
        if let Some((lru_key, _)) = cache.iter()
            .min_by_key(|(_, entry)| entry.last_accessed) {
            let lru_key = lru_key.clone();
            cache.remove(&lru_key);
    ...
    async fn evict_lru_news_cache(&self, cache: &mut HashMap<String, CacheEntry<(Vec<News>, SentimentAnalysis)>>) {
        if let Some((lru_key, _)) = cache.iter()
            .min_by_key(|(_, entry)| entry.last_accessed) {
            let lru_key = lru_key.clone();
            cache.remove(&lru_key);
...
impl<T: CachedDataFetcher> DataFetcher for CachedDataFetcherWrapper<T> {
    async fn get_stock_data(&self, stock_code: &str, days: i32) -> Result<Vec<PriceData>, String> {
        ...
        log::debug!("Cache miss for price data: {}, fetching from source", stock_code);
        
        // Fetch from source
        let data = self.inner.get_stock_data(stock_code, days).await?;
        
        // Cache the result
        self.cache.set_price_data(stock_code, days, data.clone()).await;
    ...
    async fn get_fundamental_data(&self, stock_code: &str) -> Result<FundamentalData, String> {
        ...
        log::debug!("Cache miss for fundamental data: {}, fetching from source", stock_code);
        
        // Fetch from source
        let data = self.inner.get_fundamental_data(stock_code).await?;
        
        // Cache the result
        self.cache.set_fundamental_data(stock_code, data.clone()).await;
    ...
    async fn get_news_data(&self, stock_code: &str, days: i32) -> Result<(Vec<News>, SentimentAnalysis), String> {
        ...
        log::debug!("Cache miss for news data: {}, fetching from source", stock_code);
        
        // Fetch from source
        let data = self.inner.get_news_data(stock_code, days).await?;
        
        // Cache the result
        self.cache.set_news_data(stock_code, days, data.clone()).await;
    ...
    async fn get_stock_name(&self, stock_code: &str) -> String {
        ...
        log::debug!("Cache miss for stock name: {}, fetching from source", stock_code);
