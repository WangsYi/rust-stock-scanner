...
async fn main() -> std::io::Result<()> {
    ...
    info!("Starting Rust Stock Analyzer on {}:{}", config.server.host, config.server.port);
        ...
        App::new()
            ...
            .service(
                web::scope("/api")
                    ...
                    .route("/analyze/stream", web::post().to(handlers::analyze_single_streaming))
                    .route("/batch/analyze", web::post().to(handlers::analyze_batch))
                    .route("/batch/status/{task_id}", web::get().to(handlers::get_task_status))
                    .route("/stock/{stock_code}/price", web::get().to(handlers::get_stock_price))
                    .route("/stock/{stock_code}/fundamental", web::get().to(handlers::get_stock_fundamental))
                    .route("/stock/{stock_code}/news", web::get().to(handlers::get_stock_news))
                    .route("/stock/{stock_code}/name", web::get().to(handlers::get_stock_name))
                    .route("/health", web::get().to(handlers::health_check))
                    .route("/cache/stats", web::get().to(handlers::get_cache_stats))
                    .route("/cache/clear", web::post().to(handlers::clear_cache))
                    .route("/currency/convert", web::get().to(handlers::convert_currency))
                    .route("/currency/exchange-rate", web::get().to(handlers::get_exchange_rate))
                    .route("/currency/supported", web::get().to(handlers::get_supported_currencies))
                    ...
                    .service(
                        web::scope("/configurations")
                            ...
                            .route("/{id}/activate", web::post().to(handlers::activate_configuration))
...
fn load_config() -> AppConfig {
    ...
    AppConfig {
        ...
        cache: models::CacheConfig {
            enabled: std::env::var("CACHE_ENABLED").unwrap_or_else(|_| "true".to_string()).parse().unwrap_or(true),
            price_data_ttl: std::env::var("CACHE_PRICE_TTL").unwrap_or_else(|_| "300".to_string()).parse().unwrap_or(300),
            fundamental_data_ttl: std::env::var("CACHE_FUNDAMENTAL_TTL").unwrap_or_else(|_| "3600".to_string()).parse().unwrap_or(3600),
            news_data_ttl: std::env::var("CACHE_NEWS_TTL").unwrap_or_else(|_| "1800".to_string()).parse().unwrap_or(1800),
            stock_name_ttl: std::env::var("CACHE_NAME_TTL").unwrap_or_else(|_| "86400".to_string()).parse().unwrap_or(86400),
            max_entries: std::env::var("CACHE_MAX_ENTRIES").unwrap_or_else(|_| "1000".to_string()).parse().unwrap_or(1000),
            cleanup_interval: std::env::var("CACHE_CLEANUP_INTERVAL").unwrap_or_else(|_| "60".to_string()).parse().unwrap_or(60),
            enable_stats: std::env::var("CACHE_ENABLE_STATS").unwrap_or_else(|_| "true".to_string()).parse().unwrap_or(true),
        },
    }
}